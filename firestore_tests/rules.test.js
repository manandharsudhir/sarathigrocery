...
async function seedBusiness() {
  await env.withSecurityRulesDisabled(async (ctx) => {
    ...
    await setDoc(doc(d, 'products/P1'), { name: 'Rice', unitPrice: 100, purchasePrice: 80, stockQty: 10, reservedQty: 0 });
...
describe('staff', () => {
  ...
  test('employee records a sale with its stock/balance/ledger/audit side effects in one batch', async () => {
    ...
    batch.set(doc(d, 'auditLog/A2'), { action: 'Sale recorded' });
...
describe('customers', () => {
  ...
  test('can place their own order (reserving stock) but not someone else\'s', async () => {
    const d = db(CUSTOMER);
    const batch = writeBatch(d);
    batch.set(doc(d, 'orders/O3'), { customerId: 'C1', status: 'placed' });
    batch.set(doc(d, 'products/P1'), { reservedQty: increment(2) }, { merge: true });
    batch.set(doc(d, 'auditLog/A3'), { action: 'Order placed' });
